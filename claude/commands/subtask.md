...
If this session is not working on a backlog item, there is nothing to attach the sub-task to — tell the user there is no current item and ask them to run this from a session working on one (or to add the sub-task with `assist backlog add-subtask a<id>` directly).
...
```
assist backlog add-subtask a<id> --title '<sub-task text>' 2>&1
```
